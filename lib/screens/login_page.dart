      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _R(fade: _fades[0], slide: _slides[0], child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [LuxTheme.terracotta, LuxTheme.gold], begin: Alignment.topLeft, end: Alignment.bottomRight),
            ...
            // Error banner
            if (_error != null)
              _R(fade: _fades[1], slide: _slides[1], child: Container(
            ...
            _R(fade: _fades[1], slide: _slides[1], child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 8),
              _LuxField(controller: _emailCtrl, hint: 'you@example.com', icon: Icons.mail_outline_rounded, keyboardType: TextInputType.emailAddress),
            ])),
            ...
            // Sign in button
            _R(fade: _fades[3], slide: _slides[3], child: SizedBox(
              width: double.infinity,
              child: LuxButton(label: 'Sign In', icon: Icons.login_rounded, isLoading: _loading, onTap: _loading ? null : _login),
            )),

            const SizedBox(height: 28),

            _R(fade: _fades[3], slide: _slides[3], child: const GoldDivider(label: 'OR CONTINUE WITH')),

            const SizedBox(height: 24),

            _R(fade: _fades[3], slide: _slides[3], child: Row(children: [
            ...
            // Create account
            _R(fade: _fades[4], slide: _slides[4], child: Center(child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text("Don't have an account?  ", style: LuxTheme.body.copyWith(height: 1, fontSize: 13)),
              PressScale(
                onTap: () {
                  HapticFeedback.lightImpact();
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const RegisterPage()));
                },
                child: const Text('Create one', style: TextStyle(fontSize: 13, color: LuxTheme.terracotta, fontWeight: FontWeight.w700)),
              ),
            ]))),
...
class _LuxField extends StatelessWidget {
  ...
  final Widget? suffix;
  const _LuxField({required this.controller, required this.hint, required this.icon, this.obscure = false, this.keyboardType, this.suffix});
  @override
  Widget build(BuildContext context) => TextField(
    ...
    decoration: InputDecoration(
      ...
      enabledBorder: OutlineInputBorder(borderRadius: LuxTheme.radius14, borderSide: const BorderSide(color: LuxTheme.sandDark, width: 1.2)),
...
class _SocialBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PressScale(
    ...
    child: Container(
      height: 52,
      decoration: BoxDecoration(color: LuxTheme.cream, borderRadius: LuxTheme.radius14, border: Border.all(color: LuxTheme.sandDark, width: 1.2), boxShadow: LuxTheme.cardShadow),
